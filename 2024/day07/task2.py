...
def rec(goal: int, intermediate: int, operands: list[int]) -> int:
    match operands:
        case [x]:
            return (intermediate*x == goal) \
                   or intermediate+x == goal \
                   or int(str(intermediate)+str(x)) == goal
        case [x, *xs]:
            if intermediate >= goal:
                return False
            else:
                return rec(goal, intermediate*x, xs) \
                       or rec(goal, intermediate+x, xs) \
                       or rec(goal, int(str(intermediate)+str(x)), xs)
