...
def main():
    ...
    for permutation in list(itertools.permutations([x for x in range(5)])):
        intoa = queue.SimpleQueue()
        intoa.put(permutation[0])
        intoa.put(0)
        ...
        etoout = queue.SimpleQueue()

        A = threading.Thread(target=computer, args=(
            program.copy(), intoa, atob))
